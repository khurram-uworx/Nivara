# TODO — Incident Lab core gap-fills, Phase 1 (branch `khurram/incident`)
...
**Scope (maintainer decision 2026-08-16):** only Phase 1 (core gap-fills 1.1–1.5) ships on
this branch. Phase 2 (AutoDiff 2.1–2.2), Phase 3 (sample projects), Phase 4 (bench), README
DoD, and web UI (3.5) are **deferred** to a follow-up (tracked in the issues log). After 1.5,
...
| ~2.1 Dead branch removal | `ReverseGradOperations.cs` (Gather), `GradOperationKernels.cs` (BroadcastGradient) | **DEFERRED** — tracked in issues log |
| ~2.2 `Pow` SIMD | `ReverseGradOperations.cs` (Pow) | **DEFERRED** — tracked in issues log |
...
Deferred (tracked in issues log): Phase 2 AutoDiff items (2.1 dead branch removal, 2.2 `Pow`
SIMD), Phase 3 sample projects, Phase 4 bench, README DoD, web UI (3.5).

---

## Execution order

Phase 1 core gap-fills in order 1.1 → 1.2 → 1.3 → 1.4 → 1.5.
No Phase 2/3/4 work on this branch.
...
- [ ] [#284](https://github.com/khurram-uworx/Nivara/issues/284) —
      Defer Phase 2 (AutoDiff 2.1 dead-branch removal, 2.2 `Pow` SIMD), Incident Lab
      sample/bench (Phases 3–4), web UI (3.5), README DoD to a follow-up
      (created while narrowing scope of `khurram/incident` to Phase 1 only).

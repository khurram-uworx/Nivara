...
- `QwenChatTemplate.RenderPlain(string userText)` — no-system render
  `"<|im_start|>user\n" + text + "<|im_end|>\n<|im_start|>assistant\n"`, byte-identical
  to `QwenChatClient.Generate` over a single user message (already pinned by
  `QwenChatTemplateTests.Render_PlainUserTurn_...`).
...
### 1d. Baseline-first readings record
- Real checkpoint: `qwen benchmark` (tool prompt, existing probe) + `qwen benchmark
  --plain` (new probe) → prefill ms, decode ms/token, tok/s, full-fwd. Recorded in
  `docs/QWEN.md` + perf README.
- Synthetic: perf harness `--json qwen-prefill-baseline.json --runs 3` (also serves as
  the Phase-2 gate baseline).
...
- `docs: plan qwen plain-prompt coverage + fused prefill attention in TODO.md`
- Phase 1:
  - `infer: add qwen --plain prompt benchmark and demo` (Qwen.cs + Program.cs + help)
  - `perf: record qwen baseline readings (tool + plain) before kernel work`
    (harness JSONs + QWEN.md transcript)
  - `samples: add qwen plain-prompt Torch reference generator` (qwen_plain_reference.py)
  - `tests: pin qwen plain-prompt parity (tokenizer, greedy, cache-vs-full)`
  - `docs: document qwen --plain and perf README rows`
- Phase 2:

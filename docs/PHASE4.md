...
6. **Public streaming surface (#264)** — `QueryFrame.AsStream` is public (`IAsyncEnumerable<NivaraFrame>`, `chunkSize = 10000`, `ct = default`); `NivaraQuery<T>.AsStream` passthrough added; `NivaraFrame.AsQueryFrame` / `NivaraQuery<T>.AsQueryFrame` made public; public `ScanAsQueryFrame` factories on Csv/Json/Parquet delegate to the internal `ScanFrame` implementations. Full behavior reference: `docs/STREAMING.md`.
7. **AsStream boundary behavior (#264)** — `AsStream` chunks only for fully-streamable plans (Filter/Select/Slice/SelectRows, no window expressions) over chunk-capable sources; any boundary op (Sort/SortByExpression/GroupBy/Join/Distinct/Rolling/Cumulative/Shift/Rank or a window expression) or a non-chunk-capable source yields a single merged frame with rows identical to `CollectAsync()`. This is all-or-nothing, distinct from the Collect-path flush-concatenate-resume (decision 1): chunked `AsStream` output must be independently processable, so a whole-dataset boundary op defeats chunking.

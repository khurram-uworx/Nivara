...
public static class AgentsValidatorTrainer
{
    ...
    {
        var (inputs, labels) = SyntheticDataGenerator.GenerateAgentsValidatorData(numSamples, seed);
        ...
        Console.WriteLine($"  Training agents validator: {trainCount} samples, {tokenizer.VocabSize} vocab, 2 classes");
        ...
        ModelSerializer.Save(model, Path.Combine(saveDir, "agents_validator_model.json"));
        tokenizer.Save(Path.Combine(saveDir, "agents_validator_tokenizer.json"));

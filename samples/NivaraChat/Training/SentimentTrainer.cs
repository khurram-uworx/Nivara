...
public static class SentimentTrainer
{
    ...
    {
        var (texts, labels) = SyntheticDataGenerator.GenerateSentimentData(numSamples, seed);
        ...
        using var model = new TextClassifierModel<float>(tokenizer.VocabSize, 32, 64, numClasses: 3, maxSeqLen);
        ...
        Console.WriteLine($"  Training sentiment: {trainCount} samples, {tokenizer.VocabSize} vocab, 3 classes");

        var trainLoop = new TrainingLoop<float>(
            model, trainLoader,
            (logits, lbls) =>
            {
                int bs = logits.Length / 3;
        ...
        ModelSerializer.Save(model, Path.Combine(saveDir, "sentiment_model.json"));
        tokenizer.Save(Path.Combine(saveDir, "sentiment_tokenizer.json"));

...
public static class TensorExtensions
{
        ...
        where T : struct, INumber<T>
    {
        ...
        {
            var leftSpan = MemoryMarshal.Cast<T, float>(left.Values);
            var rightSpan = MemoryMarshal.Cast<T, float>(right.Values);
            ...
            return new NivaraSeries<T>(resultT);
        }
        else if (typeof(T) == typeof(double))
        {
            var leftSpan = MemoryMarshal.Cast<T, double>(left.Values);
            var rightSpan = MemoryMarshal.Cast<T, double>(right.Values);
        ...
        where T : struct, INumber<T>
    {
        ...
        if (typeof(T) == typeof(float))
        {
            var span = MemoryMarshal.Cast<T, float>(series.Values);
            var result = TensorPrimitives.Sum(span);
            return (T)(object)result;
        }
        else if (typeof(T) == typeof(double))
        {
            var span = MemoryMarshal.Cast<T, double>(series.Values);
            var result = TensorPrimitives.Sum(span);
            return (T)(object)result;
        }
        else
        ...
        where T : struct, INumber<T>
    {
        ...
        else
        {
            ...
            for (int i = 0; i < left.Length; i++)
            {
                var product = left.GetRawValue(i) * right.GetRawValue(i);
                result += product;
            }
            return result;
        }
        ...
        where T : struct, INumber<T>
    {
        ...
        else
        {
            ...
            for (int i = 0; i < series.Length; i++)
            {
                var value = series.GetRawValue(i);
        ...
        where T : struct, INumber<T>
    {
        ...
        return new NivaraSeries<T>(result);
    }

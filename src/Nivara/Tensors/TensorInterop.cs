...
public static class TensorInterop
{
        ...
        where T : struct, INumber<T>
    {
        ...
        {
            data[i] = series.IsValid(i) ? series.GetRawValue(i) : default(T);
        }
        ...
        where T : struct, INumber<T>
    {
        ...
        {
            data[i] = tensorSpan[i];
            validity[i] = true; // All tensor values are considered valid
        }

        return new NivaraSeries<T>(data, validity);
    }
        ...
        where T : struct, INumber<T>
    {
        ...
        var validityMask = series.ValidityMask;
        for (int i = 0; i < validityMask.Length; i++)
        {
            if (!validityMask[i])
            {
        ...
        // Create TensorSpan with shared memory
        ...
        return new TensorSpan<T>(span, dimensions, default);
    }
        ...
        where T : struct, INumber<T>
    {
        ...
        foreach (var columnType in frame.ColumnTypes)
        {
        ...
        for (int row = 0; row < frame.RowCount; row++)
        {
            for (int col = 0; col < frame.ColumnCount; col++)
            {
                ...
                data[row * frame.ColumnCount + col] = series.GetRawValue(row);
            }
        }
        ...
        where T : struct, INumber<T>
    {
        ...
        if (rows == 0 || cols == 0)
        {
            return new NivaraFrame();
        }

        var columns = new Dictionary<string, INivaraColumn>();
        var tensorSpan = tensor.AsTensorSpan();
        ...
        for (int col = 0; col < cols; col++)
        {
            ...
            var series = new NivaraSeries<T>(columnData);
            columns[columnNames[col]] = new NivaraColumn<T>(series);
        }
        ...
        where T : struct, INumber<T>
    {
        ...
        var validity = new bool[totalElements];
        ...
        // All tensor values are considered valid
        ...
        where T : struct, INumber<T>
    {
        ...
        var data = new T[totalElements];
        var seriesValues = series.Values;

        // Copy data from series
        seriesValues.CopyTo(data);

...
/// <summary>
/// Interface for gradient-aware operations in automatic differentiation.
...
public interface IGradOperation<T> where T : struct, INumber<T>
{
    ...
    /// </summary>
    /// <param name="inputs">The input GradTensors for the operation</param>
    /// <returns>The result of the forward computation as a GradTensor</returns>
    GradTensor<T> Forward(params GradTensor<T>[] inputs);
    ...
    void Backward(NivaraColumn<T> gradOutput, GradTensor<T>[] inputs, GradTensor<T> output);
}
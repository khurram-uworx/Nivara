...
- ✓ **IFloatingPointIeee754<T>**: AutoDiff type constraint relaxed from `INumber<T>`; `Half`/F16 passes runtime validation alongside `float`/`double`. All AutoDiff ops span-ified with `TensorPrimitives`. Note: `System.Numerics.BFloat16` exists only on .NET 11, so BFloat16-typed kernels are untestable at the net10.0 TFM; today's BFloat16 support is load-time BF16→F32 widening in `SafeTensorsLoader`. Kernel tests for BFloat16 are deferred to the net11 migration.
